...
function register_meta_for_wp_rest_api() {
  ...
  register_meta('post', 'attention', $args_register_post_meta);
  register_meta('post', 'svg', $args_register_post_meta);
  register_meta('post', 'gradation', $args_register_post_meta);
        ...
        switch($key):
          ...
          case 'meta':
            foreach((array)$value as $meta => $field):
              switch($meta):
                case 'gradation':
                  $response->data[$key][$meta] = get_field('gradation', $post->ID);
                  break;
                case 'price':
                ...
                case 'codes':
                  ...
                  while(have_rows($meta, $post->ID)): the_row();
                    ...
                    $data = explode(',', get_sub_field('data'));
                    $i = 0;
                    foreach((array)$data as $col => $val):
                      if(!$val) continue;
                      if($i == 0 || $i%2 == 0):
                        $codes1[$name][$i]['label'] = $val;
                      else:
                        $codes1[$name][$i-1]['hex'] = $val;
                      endif;
                      $i++;
                    endforeach;
                    //var_dump($codes1);
                    ...
                    */
                  endwhile;
                  foreach((array)$codes1 as $code => $info):
                    foreach((array)$info as $col2 => $val2):
                      $codes2[$code][] = array('label' => $val2['label'], 'hex' => $val2['hex']);
                    endforeach;
                    /*
                    $codes2[$code][] = array('label' => $info['label'], 'hex' => $info['hex']);
                    */
                  endforeach;
                  $response->data[$key][$meta] = $codes2;
                  break;
                default:
                  break;
              endswitch;
