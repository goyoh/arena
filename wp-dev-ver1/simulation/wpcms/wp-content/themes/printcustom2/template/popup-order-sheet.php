    ...
    foreach((array)$spec as $colum => $data):
      switch($keys[$colum]):
        ...
        case 'colA':
          ...
          break;
        case 'ecolA':
          $term = get_term_by('slug', $data, 'mark-ecolor');
          //$txt .= ($term)? $term->name . ', ' : null;
          break;
        case 'markA':
...
?>
  <li class="order-sheet__item" data-number="<?php echo $i; ?>" data-style="<?php echo $data; ?>">
    <div class="order-sheet__image">
      <?php if(is_array($spec)): ?>
      <iframe src="/simulation/printcustom2/item/?type=<?php echo $spec[0]; ?>&bcol=<?php echo $spec[1]; ?>" id="" name="" title="" width="58px" height="58px" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe>
      <?php else: ?>
